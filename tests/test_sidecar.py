...
def test_mtime_cache_invalidates_when_file_rewritten(sidecar_dir: pathlib.Path):
    ...
    path.write_text(json.dumps({"ticket": "PROJ-123"}))
    assert routes.read_session_sidecar("abc") == {"ticket": "PROJ-123"}
    # Bump mtime explicitly so the test isn't flaky on coarse filesystem
    # timestamp resolution.
    later = time.time() + 10
    path.write_text(json.dumps({"ticket": "PROJ-456"}))
    os.utime(path, (later, later))
    assert routes.read_session_sidecar("abc") == {"ticket": "PROJ-456"}
