import { describe, it, expect } from "vitest";
import { render, screen, fireEvent, within } from "@testing-library/react";
import { UsageList } from "./UsageList";
import type { ProjectRow, SessionRow, TopicRow } from "../types";

const topicRows: TopicRow[] = [
  { topic_id: "PROJ-100", label: "PROJ-100", sessions: 2, output: 100,
    input: 10, messages: 5, last_at: null, summary: null, sample_prompts: [] },
  { topic_id: "PROJ-200", label: "PROJ-200", sessions: 1, output: 500,
    input: 50, messages: 20, last_at: null, summary: null, sample_prompts: [] },
  { topic_id: "PROJ-300", label: "PROJ-300", sessions: 3, output: 250,
    input: 25, messages: 10, last_at: null, summary: null, sample_prompts: [] },
];

const projectRows: ProjectRow[] = [
  { project: "alpha", sessions: 1, output: 100, input: 0, messages: 1, last_at: null, sample_prompts: [] },
  { project: "beta", sessions: 1, output: 50, input: 0, messages: 1, last_at: null, sample_prompts: [] },
];

function getRowOrder(): string[] {
  const tbody = document.querySelector("tbody")!;
  const rows = within(tbody).queryAllByRole("row");
  return rows.map((r) => r.textContent ?? "");
}

describe("UsageList", () => {
  it("defaults to sorting by output desc", () => {
    render(<UsageList by="topic" rows={topicRows} />);
    const order = getRowOrder();
    expect(order[0]).toContain("PROJ-200"); // 500 (highest)
    expect(order[1]).toContain("PROJ-300"); // 250
    expect(order[2]).toContain("PROJ-100"); // 100
  });

  it("flips sort direction when the active header is clicked again", () => {
    render(<UsageList by="topic" rows={topicRows} />);
    fireEvent.click(screen.getByText("Output")); // active header — flips to asc
    const order = getRowOrder();
    expect(order[0]).toContain("PROJ-100"); // 100 (lowest)
    expect(order[2]).toContain("PROJ-200"); // 500 (highest)
  });

  it("switches sort key when a different header is clicked", () => {
    render(<UsageList by="topic" rows={topicRows} />);
    fireEvent.click(screen.getByText("Sessions")); // becomes active, dir=desc
    const order = getRowOrder();
    expect(order[0]).toContain("PROJ-300"); // 3 sessions
  });

  it("renders project columns when by=project", () => {
    render(<UsageList by="project" rows={projectRows} />);
    expect(screen.getByText("Project folder")).toBeInTheDocument();
    expect(screen.queryByText("Topic")).not.toBeInTheDocument();
  });

  it("shows the empty state when rows is empty", () => {
    render(<UsageList by="topic" rows={[]} />);
    expect(screen.getByText("no data yet")).toBeInTheDocument();
  });

  it("renders dominant topic only when session has a single segment", () => {
    const session: SessionRow = {
      session_id: "abc-1234", project: "myrepo",
      output: 100, input: 10, messages: 3,
      started_at: null, last_at: null, early_user_prompts: [],
      topic_id: "PROJ-144",
      segments: { "PROJ-144": { output: 100, input: 10, messages: 3, last_at: null } },
    };
    render(<UsageList by="session" rows={[session]} />);
    expect(screen.getByText("PROJ-144")).toBeInTheDocument();
    expect(screen.queryByText(/\+/)).not.toBeInTheDocument();
  });

  it("renders dominant + next when session has multiple segments", () => {
    const session: SessionRow = {
      session_id: "abc-1234", project: "myrepo",
      output: 200, input: 20, messages: 6,
      started_at: null, last_at: null, early_user_prompts: [],
      topic_id: "PROJ-144",
      segments: {
        "PROJ-144": { output: 130, input: 13, messages: 4, last_at: null },
        "PROJ-119": { output: 70, input: 7, messages: 2, last_at: null },
      },
    };
    render(<UsageList by="session" rows={[session]} />);
    expect(screen.getByText("PROJ-144")).toBeInTheDocument();
    expect(screen.getByText(/\+ PROJ-119/)).toBeInTheDocument();
  });

  it("renders the topic summary inline when present", () => {
    const rows: TopicRow[] = [
      { topic_id: "PROJ-144", label: "PROJ-144", sessions: 1, output: 100,
        input: 0, messages: 1, last_at: null,
        summary: "example ticket summary", sample_prompts: [] },
    ];
    render(<UsageList by="topic" rows={rows} />);
    expect(screen.getByText(/example ticket summary/)).toBeInTheDocument();
  });

  it("renders only the topic id when summary is null", () => {
    const rows: TopicRow[] = [
      { topic_id: "PROJ-144", label: "PROJ-144", sessions: 1, output: 100,
        input: 0, messages: 1, last_at: null, summary: null, sample_prompts: [] },
    ];
    render(<UsageList by="topic" rows={rows} />);
    expect(screen.getByText("PROJ-144")).toBeInTheDocument();
    expect(screen.queryByText(/·/)).not.toBeInTheDocument();
  });

  it("renders +N more suffix when session has 3+ segments", () => {
    const session: SessionRow = {
      session_id: "abc-1234", project: "myrepo",
      output: 300, input: 30, messages: 9,
      started_at: null, last_at: null, early_user_prompts: [],
      topic_id: "PROJ-144",
      segments: {
        "PROJ-144": { output: 150, input: 15, messages: 4, last_at: null },
        "PROJ-119": { output: 100, input: 10, messages: 3, last_at: null },
        "PROJ-200": { output: 50,  input: 5,  messages: 2, last_at: null },
      },
    };
    render(<UsageList by="session" rows={[session]} />);
    expect(screen.getByText(/\+ PROJ-119/)).toBeInTheDocument();
    expect(screen.getByText(/\(\+1\)/)).toBeInTheDocument();
  });
});
